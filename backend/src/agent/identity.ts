...
export function agentIdentity(): AgentIdentity {
    ...
    if (!st.isFile() || st.mode & 0o077)
        throw new Error('Agent identity must be a private 0600 file');
    ...
    if (
        process.env.GAMEPANEL_NODE_ID &&
        process.env.GAMEPANEL_NODE_ID !== value.nodeId
    )
        throw new Error('Agent node identity mismatch');
    process.env.GAMEPANEL_NODE_ID = value.nodeId;
    cached = {
        ...value,
        panel: nodeOrigin(value.panel, loopback),
        origin: nodeOrigin(value.origin, loopback),
    };
    return cached!;
}
