...
export class OperationJournal {
    ...
    async admit(
        ...
        const result = await this.db.run(
            'INSERT OR IGNORE INTO agent_requests(id,fingerprint,state,created_at) VALUES(?,?,?,?)',
            id,
            fingerprint,
            'running',
            Date.now(),
        ...
        const previous = await this.db.get<Operation>(
    ...
    async complete(id: string, status: number, response: string) {
        await this.db.run(
            "UPDATE agent_requests SET state='completed',status=?,response=? WHERE id=?",
            status,
            response,
            id,
        );
    ...
    async uncertain(id: string) {
        await this.db.run(
            "UPDATE agent_requests SET state='uncertain' WHERE id=?",
            id,
        );
    }
    async lookup(id: string) {
        ...
        return this.db.get(
            'SELECT id,state,status,created_at FROM agent_requests WHERE id=?',
            id,
        );
    }
}
