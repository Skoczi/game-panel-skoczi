...
import { serverRepository, serverMemberRepository, userRepository } from '../database/index.js';
...
async function readInventory(id: string): Promise<InventoryItem[]> {
    ...
    return new Promise((resolve, reject) => {
        const req = (url.protocol === 'https:' ? https : http).request(
            ...
            (res) => {
                ...
                res.on('data', (chunk: Buffer) => {
                    bytes += chunk.length;
                    if (bytes > 16 * 1024 * 1024) req.destroy(new Error('Inventory too large'));
                ...
                res.on('end', () => {
                    try {
                        ...
                        if (!Array.isArray(value.servers)) throw new Error('Invalid inventory');
        ...
        const deadline = setTimeout(() => req.destroy(new Error('Inventory timeout')), 10000);
    ...
    refreshing ??= (async () => {
        ...
        await Promise.all(
            Array.from({ length: Math.min(4, remaining.length) }, async () => {
                for (let node = remaining.shift(); node; node = remaining.shift()) {
                    try {
                        await store.observe(node.id, await readInventory(node.id));
...
export async function fleetPermissions(
    ...
    if (
        row.node_id === 'local' &&
        (await serverRepository.findById(row.runtime_id))?.runtime_uuid !== row.runtime_key
    )
        return null;
    if (user.isRoot) return [...ASSIGNABLE_SERVER_PERMISSIONS];
    if (row.node_id !== 'local') return store.permissions(row.id, user.userId);
    const member = await serverMemberRepository.find(row.runtime_id, user.userId);
    if (!member) return null;
    const permissions = await serverMemberRepository.getUserServerPermissions(
        row.runtime_id,
        user.userId,
    );
    return permissions.includes('*') ? [...ASSIGNABLE_SERVER_PERMISSIONS] : permissions;
}
export async function serverDelegation(
    ...
    if (!row || row.missing || row.node_id !== nodeId) throw new Error('Server unavailable');
...
export function mountFleet(app: express.Application) {
    ...
    router.get(
        ...
        safe(async (req, res) => {
            ...
            const permissions = row && !row.missing ? await fleetPermissions(row, req.user!) : null;
            if (!row || permissions === null)
                return res.status(404).json({ error: 'Server not found' });
            const node = row.node_id === 'local' ? null : await nodes().get(row.node_id);
            if (row.node_id !== 'local' && (!node?.enabled || !node.key_encrypted))
    ...
    router.put(
        ...
        safe(async (req, res) => {
            ...
            if (
                ...
                return res.status(404).json({ error: 'Server or user not found' });
            ...
            if (
                ...
                return res.status(400).json({ error: 'Invalid server permissions' });
            if (row.node_id === 'local') {
                if (await serverMemberRepository.find(row.runtime_id, userId))
                    await serverMemberRepository.update(row.runtime_id, userId, permissions);
                else await serverMemberRepository.create(row.runtime_id, userId, permissions);
                await store.audit(
                    row.id,
                    req.user!.userId,
                    `grant:${userId}:${JSON.stringify(permissions)}`,
                );
            } else await store.grant(row.id, userId, permissions, req.user!.userId);
            res.json({ ok: true });
        }),
    );
    router.delete(
        ...
        safe(async (req, res) => {
            const row = await store.get(req.params.id);
            const userId = Number(req.params.userId);
            if (!row || !Number.isSafeInteger(userId) || userId <= 0)
                return res.status(404).json({ error: 'Server not found' });
            if (row.node_id === 'local') {
