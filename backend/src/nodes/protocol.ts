...
import { validateDelegation, type Delegation } from './delegation.js';
...
export function seal(value: string, master: string, nodeId: string): string {
    const key = createHash('sha256')
        .update('gamepanel-node-key-v1\0' + master)
        .digest();
    ...
    return jwt.sign(
        {
            ...
            path,
            actor,
            protocol: delegation ? 2 : PROTOCOL_VERSION,
            ...(delegation ? { delegation: validateDelegation(delegation) } : {}),
        },
        key,
        {
            algorithm: 'HS256',
...
export class RequestVerifier {
    ...
    verify(token: string, key: string, nodeId: string, method: string, path: string): NodeClaim {
        ...
        if (
            typeof value !== 'object' ||
            (value.protocol !== PROTOCOL_VERSION && value.protocol !== 2) ||
            value.method !== method ||
            value.path !== path ||
            typeof value.actor !== 'string' ||
            value.actor.length > 100 ||
            !value.actor ||
            !value.jti ||
            !value.exp
        )
            throw new Error('Invalid node claim');
        if (value.protocol === 2) value.delegation = validateDelegation(value.delegation);
        else if (value.delegation !== undefined) throw new Error('Delegation requires protocol 2');
        const now = Date.now() / 1000;
        for (const [id, expires] of this.seen) if (expires < now) this.seen.delete(id);
        if (this.seen.has(value.jti) || this.seen.size >= 20000)
            throw new Error('Replayed request or replay cache full');
