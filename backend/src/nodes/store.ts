...
export type NodeRow = {
    ...
    enrollment_expires: number | null;
...
export type PublicNode = Omit<
    NodeRow,
    'key_encrypted' | 'enrollment_hash' | 'enrollment_expires'
> & {
...
export class NodeStore {
    ...
    async audit(nodeId: string, actor: string, action: string) {
        await this.db.run(
            'INSERT INTO node_audit(node_id,actor,action,created_at) VALUES(?,?,?,?)',
            nodeId,
            actor,
            action,
            Date.now(),
    ...
    async get(id: string): Promise<NodeRow | undefined> {
        return this.db.get('SELECT * FROM execution_nodes WHERE id=?', id);
    }
    public(row: NodeRow): PublicNode {
        const {
            key_encrypted,
            enrollment_hash: _hash,
            enrollment_expires: _expiry,
            ...safe
        } = row;
        return {
            ...safe,
            status: !row.enabled
                ...
                : !key_encrypted
                  ? 'pending'
                  : row.last_seen && Date.now() - row.last_seen < 60000
                    ? 'online'
                    : 'offline',
        };
    }
    async list(): Promise<PublicNode[]> {
        return (
            await this.db.all<NodeRow[]>(
                'SELECT * FROM execution_nodes ORDER BY created_at',
            )
        ).map((row) => this.public(row));
    }
    async create(
        input: { name: string; origin: string; location?: string },
        actor: string,
    ) {
        const name = input.name?.trim();
        const location = input.location?.trim() || '';
        if (!name || name.length > 80 || location.length > 120)
            throw new Error('Name (1–80) and location (0–120) required');
        const origin = nodeOrigin(input.origin, this.allowLoopback);
        const id = randomUUID();
        const token = secret();
        await this.db.run(
            'INSERT INTO execution_nodes(id,name,origin,location,enrollment_hash,enrollment_expires,created_at) VALUES(?,?,?,?,?,?,?)',
            id,
            name,
            origin,
            location,
        ...
        return {
            node: this.public((await this.get(id))!),
    ...
    async enroll(id: string, token: string) {
        ...
        if (
            !row?.enabled ||
            !row.enrollment_hash ||
            !row.enrollment_expires ||
            row.enrollment_expires < Date.now() ||
            !timingSafeEqual(
                Buffer.from(hash),
                Buffer.from(row.enrollment_hash),
            )
        )
            throw new Error('Invalid or expired enrollment');
        const key = secret();
        // Atomic token consumption; parallel requests cannot enroll twice.
        const changed = await this.db.run(
            'UPDATE execution_nodes SET key_encrypted=?,enrollment_hash=NULL,enrollment_expires=NULL,last_seen=NULL WHERE id=? AND enrollment_hash=? AND enrollment_expires>? AND enabled=1',
            seal(key, this.master, id),
            id,
            hash,
            Date.now(),
        );
    ...
    key(row: NodeRow): string {
        if (!row.enabled || !row.key_encrypted)
            throw new Error('Node disabled or not enrolled');
    ...
    async setEnabled(id: string, enabled: boolean, actor: string) {
        ...
        await this.db.run(
            'UPDATE execution_nodes SET enabled=? WHERE id=?',
            enabled ? 1 : 0,
            id,
        );
    ...
    async renewEnrollment(id: string, actor: string) {
        ...
        const token = secret();
        await this.db.run(
            'UPDATE execution_nodes SET key_encrypted=NULL,enrollment_hash=?,enrollment_expires=?,last_seen=NULL WHERE id=?',
            digest(token),
            Date.now() + 900000,
            id,
        );
    ...
    async heartbeat(id: string, version: string) {
        await this.db.run(
            'UPDATE execution_nodes SET last_seen=?,agent_version=? WHERE id=?',
            Date.now(),
            version.slice(0, 80),
            id,
        );
