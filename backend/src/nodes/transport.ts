    ...
    upstream.on('response', (remote) => {
        if (
            ...
            remote.on('data', (chunk: Buffer) => {
                size += chunk.length;
                if (size > 16384) {
                    upstream.destroy(new Error('Oversized agent response'));
                    return;
                }
                chunks.push(chunk);
            });
            ...
            remote.on('end', () => {
                ...
                try {
                    const value = options.transformJson!(
        ...
        if (
            ...
            remote.statusCode !== 304
        ) {
            remote.resume();
            res.writeHead(502, { 'Content-Type': 'application/json' });
            res.end('{"error":"Agent redirect refused"}');
            return;
        }
        res.writeHead(remote.statusCode || 502, {
            ...filteredHeaders(remote.headers),
            'Cache-Control': 'no-store',
            'X-GamePanel-Node': options.nodeId,
        });
        pipeline(remote, res, () => {});
    });
    upstream.on('error', () => {
        ...
        res.end(
            JSON.stringify({
                error: 'Node unavailable. No local fallback. If this was a mutation, verify its result before retrying.',
