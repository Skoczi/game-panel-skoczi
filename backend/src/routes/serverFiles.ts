        ...
        try {
            ...
            if (contentLength > SMALL_UPLOAD_LIMIT_BYTES) {
                return res.status(413).json({
                    error: 'File is too large for direct upload; use upload sessions',
                });
            }
...
router.put(
    '/upload-sessions/:uploadId/chunks',
    requireServerPermission(PERMISSIONS.fs.write),
    async (req: AuthenticatedRequest, res: Response) => {
        try {
            const serverId = requirePositiveInt(req.params.id, 'Invalid server id');
...
router.post(
    '/upload-sessions/:uploadId/complete',
    requireServerPermission(PERMISSIONS.fs.write),
    async (req: AuthenticatedRequest, res: Response) => {
        try {
            const serverId = requirePositiveInt(req.params.id, 'Invalid server id');
...
router.post(
    ...
    async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            const resolvedBase = await resolveServerPath({
                serverId,
                root,
                path: basePath,
            });
...
router.post(
    ...
    async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            const absFrom = await resolveServerPath({
                serverId,
                root: fromRoot,
                path: from,
            });
            const absTo = await resolveServerPath({
                serverId,
                root: toRoot,
                path: to,
            });
    ...
    async (req: AuthenticatedRequest, res: Response) => {
        try {
            ...
            const paths = Array.isArray(body.paths)
                ? stringArray(body.paths, 'paths must be an array of strings')
                : [];
            if (paths.length === 0) {
