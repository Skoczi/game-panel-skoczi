...
export function renderFastDownloadPage(
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>${escape(title)} · Game Panel</title><style>${css}</style></head><body><main class="shell"><div class="brand">${icon("download")} GAME PANEL</div><header class="hero"><div><h1>${escape(title)}</h1>${listing ? "" : `<p class="subtitle">FastDownload · ${code}</p>`}</div></header>${body}<footer class="footer"><span>FastDownload${id ? " · Server " + id : ""}</span></footer></main></body></html>`;
}
