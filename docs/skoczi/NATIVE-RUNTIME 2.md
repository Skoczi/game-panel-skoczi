...
- Activity records step names and outcomes. Installer stdout/stderr is streamed through the existing authorized log subscription and a redacted tail is stored in SQLite before maintenance-container cleanup (up to 2,000 lines / 512 KiB, 4 KiB per line). Declared secret values and common credential assignments are masked; recipes must still avoid printing credentials. Agent interruption retains already flushed output, not necessarily the final in-flight lines.
- Native recipes share the installation progress modal, including named recipe steps, polling/reconnection and per-tab resumption when reopening the template. Percentages describe orchestration stages, not download bytes. Runtime logs replace the live installer source when the game starts; retained installer history is included on resubscription.
- Native console commands use the running, node-owned container's stdin, never a host shell or LinuxGSM. Game replies arrive through normal logs. Games without a stdin command interface still provide stdout/stderr logs; accepting bytes is not proof that a game understood a command. Custom images without a validated native snapshot remain unsupported for command dispatch.
...
No automatic conversion of eggs, migration of LinuxGSM data, per-variable user edit permissions, managed ReHLDS releases, game binary mirror, signed artifact registry, or automatic rollback is included in this preview. Docker Engine administrators can bypass panel locks; one agent process must own each runtime database.
