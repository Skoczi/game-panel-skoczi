...
- Footer: **Game Panel · Skoczi Edition**, then **v1.5.0 · Revision 7**, followed by a **Based on OVHcloud Game Panel** link to the original repository. The version line separates the upstream base from the fork revision. The technical version (`1.5.0-skoczi.7`) remains in package metadata, tags, update checks and tooltips.
