...
export function NodeSelector() {
  ...
  useEffect(() => {
    let active = true;
    const refresh = () => { void nodesRequest<{ nodes: ExecutionNode[] }>('/api/nodes').then(result => {
      if (active) { setNodes(result.nodes); setError(false); }
  ...
  return <div className="border-b border-white/15 px-3 py-3 text-white">
    ...
    <select id="active-node" value={ACTIVE_NODE} className="mt-2 w-full rounded-lg border border-white/20 bg-[#071b70] p-2 text-xs text-white"
      ...
      {ACTIVE_NODE !== 'local' && !nodes.some(node => node.id === ACTIVE_NODE) && <option value={ACTIVE_NODE}>Selected node unavailable</option>}
      {nodes.map(node => <option key={node.id} value={node.id}>{node.name} · {node.status}</option>)}
    </select>
    {error && <p role="status" className="mt-2 text-xs text-amber-200">Node list unavailable. Current selection retained.</p>}
  </div>;
}
