...
import localReleaseNotes from '../../docs/pro/RELEASE-2.0.55.md?raw';
    ...
    <AppModal open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <AppModalContent className="gp-panel-update max-h-[90vh] w-[calc(100%-2rem)] max-w-2xl overflow-hidden">
        ...
        <AppModalBody className="space-y-4 overflow-y-auto">
          ...
          {getAppVersion() === '2.0.55' && <details open className="text-sm"><summary className="cursor-pointer font-medium">Installed changelog · 2.0.55</summary><Markdown>{localReleaseNotes}</Markdown></details>}
          ...
          {info?.currentRelease && getAppVersion() !== '2.0.55' && <ReleaseNotesBlock release={info.currentRelease} isDark={isDark} heading="Installed release" />}
