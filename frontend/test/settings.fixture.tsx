...
function Fixture() {
  ...
  if (new URLSearchParams(location.search).has('login')) return <BrandingProvider><Login onLogin={() => {}} /></BrandingProvider>;
  return <BrandingProvider><ThemeProvider><div className="min-h-screen bg-gray-50 dark:bg-[#0b111b] md:pl-52">
    <div className="hidden md:block"><Sidebar activeTab={tab} onTabChange={setTab} currentUser={{ id: 1, username: 'Admin', isRoot } as any} /></div>
    <main className="min-w-0 p-4 md:p-6"><NewsPanel />{isRoot && tab === 'settings' ? <GlobalSettings /> : <p>Game Servers</p>}</main>
  </div></ThemeProvider></BrandingProvider>;
}
