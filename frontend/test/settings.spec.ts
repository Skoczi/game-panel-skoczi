...
test('global settings only edits appearance and preserves Local allocations', async ({ page }) => {
  ...
  await expect(revision).toHaveText('v1.5.0 · Revision 16');
  ...
  await expect(revision.locator('..')).toHaveAttribute('title', /1\.5\.0-skoczi\.16/);
...
test('non-root menu has no global Settings entry', async ({ page }) => {
  ...
  await expect(page.getByTestId('panel-revision')).toHaveText('v1.5.0 · Revision 16');
