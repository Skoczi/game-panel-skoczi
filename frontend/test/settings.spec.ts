...
test('global settings only edits appearance and preserves Local allocations', async ({ page }) => {
  ...
  await expect(revision).toHaveText('v1.5.0 · Revision 15');
  ...
  await expect(revision.locator('..')).toHaveAttribute('title', /1\.5\.0-skoczi\.15/);
...
test('non-root menu has no global Settings entry', async ({ page }) => {
  ...
  await expect(page.getByTestId('panel-revision')).toHaveText('v1.5.0 · Revision 15');
