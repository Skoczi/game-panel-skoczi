...
test('global settings only edits appearance and preserves Local allocations', async ({ page }) => {
  ...
  await expect(revision.locator('..')).toHaveAttribute(
    'title',
    `Version ${packageInfo.version} — Panel update`
  );
...
test('version dialog keeps bundled notes offline and disables updates on unmanaged installations', async ({ page }) => {
  ...
  await expect(dialog.getByText('Installed changelog · 2.0.53')).toBeVisible();
  ...
  await expect(dialog.getByText(/Renamed Startup & Settings/)).toBeVisible();
