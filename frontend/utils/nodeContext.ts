...
export const ACTIVE_NODE = (() => {
  try { const value = sessionStorage.getItem(KEY); return value && valid.test(value) ? value : 'local'; }
  catch { return 'local'; }
})();
export function selectNode(id: string) {
  ...
  sessionStorage.setItem(KEY, id); clearAppCache(); window.location.reload();
}
export function clearNodeSelection() { sessionStorage.removeItem(KEY); }
export function runtimeUrl(url: string, nodeId = ACTIVE_NODE): string {
  ...
  if (/^\/api\/servers(?:\/|$)/.test(url) || url === '/api/system/bind-addresses') return `/api/nodes/${nodeId}/runtime${url}`;
  return url;
}
