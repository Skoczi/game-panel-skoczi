...
test ! -e /data/hlds_linux || { echo 'Existing installation detected; refusing to overwrite it'; exit 2; }
...
for attempt in 1 2 3; do
  if ./linux32/steamcmd +force_install_dir /data +login anonymous +app_set_config 90 mod cstrike +app_update 90 -beta steam_legacy validate +quit; then
    if [[ -s /data/cstrike/dlls/cs.so && -s /data/cstrike/maps/de_dust2.bsp && -s /data/valve/valve.rc ]]; then complete=1; break; fi
  fi
  echo "Incomplete Steam download, attempt $attempt/3"
done
(( complete == 1 )) || { echo 'Steam download incomplete; installation stopped'; exit 1; }
install -m 755 "$stage/bin/linux32/engine_i486.so" /data/engine_i486.so
install -m 755 "$stage/bin/linux32/filesystem_stdio.so" /data/filesystem_stdio.so
install -m 755 "$stage/bin/linux32/hlds_linux" /data/hlds_linux
mkdir -p /data/.steam/sdk32
install -m 644 linux32/steamclient.so /data/.steam/sdk32/steamclient.so
printf '%s\n' 'ReHLDS 3.15.0.896' > /data/.gamepanel-engine-version
